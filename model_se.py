...
class ConvLayer(nn.Module):
    def __init__(
            ...
            use_residual: Optional[bool] = False,  # 添加使用残差连接的选项
    ) -> None:  # 无返回值
        super().__init__()
        ...
        # self.conv1d = nn.Conv2d(64, 48, 1, 1)
    ...
    # 前向推理函数
    def forward(self, x: Tensor, flag=0) -> Tensor:
        if flag == 0:
            return self.block(x)
        else:
            x = self.conv1d(x)
...
class SEModule(nn.Module):
    def __init__(self, channel, reduction=4):
        ...
        self.conv1x1 = nn.Conv2d(channel, 1, kernel_size=1)
...
class InvertedResidual(nn.Module):
    ...
    def __init__(
            ...
            # 逆残差结构的第一个点卷积升维的倍数，该参数的作用是保持特征图深度不变的同时，增加特征图的通道数
            expand_ratio: Union[int, float],

            # 是否使用残差连接，即跳过网络内部结构的变化，直接将输入添加到网络的输出中，有助于缓解梯度消失
        ...
        if stride == 1:
            # 当步长为1时，表示不进行下采样，在Inverted Residual结构中添加一个卷积层conv_3x3
            block.add_module(
                name="conv_3x3",
                module=ConvLayer(
...
class DepthwiseSeparableConvLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1):
        ...
        self.depthwise_conv = nn.Conv2d(
            ...
            stride=stride,
            groups=in_channels,
            padding=kernel_size // 2,  # Assuming kernel_size is odd
            bias=False
        )
...
class MobileViTBlock(nn.Module):
    def __init__(
        ...
        conv_3x3_out = DepthwiseSeparableConvLayer(
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # patches, _ = self.spatial_attention(patches, patches, patches)
...
class MobileViT(nn.Module):
    ...
    def __init__(self, model_cfg: Dict, num_classes: int = 1000):
        ...
        self.conv_2 = nn.Conv2d(80, 20, kernel_size=1)

        # 980 = 20 * 7 * 7 是自己决定的值，最好 < 1000
        self.fcimage = nn.Linear(980, 100)
        self.fchp = nn.Linear(3, 100)

        # 全连接层，拼接这两个向量
        self.fc = nn.Linear(100, 980)
        ...
        # MobileViT模型结构中的最后一个1x1卷积层，in_channels改为20
    ...
    def forward(self, x: Tensor, hp) -> Tensor:
        x = self.conv_1(x)
        x = self.layer_1(x)
        x = self.layer_2(x)
        x = self.layer_3(x)
        ...
        x = self.layer_5(x)
        """
        获取到xout和hpout后，可选的拼接操作有很多，这里的"+"只是其中一个示例
        """
        # 使用1*1的卷积降低通道数
        x = self.conv_2(x)
        x = x.view(x.size(0), -1)
        ...
        x = self.fc(x)
        x = x.view(x.size(0), 20, 7, 7)  # 固定值
        x = self.conv_1x1_exp(x)
        x = self.classifier(x)
        return x

    def forwardCopy(self, x: Tensor) -> Tensor:
        x = self.conv_1(x)
        print(x.shape)  # 打印出x的维度，维度相乘以后就是fcimage的第一个参数
...
#     mobile_model = mobile_vit_xx_small(num_classes=1000)
#     input_size = (1, 3, 224, 224)  # 输入大小，这里假设是一个批量的3通道224x224图像
#     flops, params = thop.profile(mobile_model, inputs=(torch.randn(input_size),))
